/**
 * MANOLITâˆž FORESTAL - SIMULADOR CUÃNTICO DE ESTRÃ‰S DE BIOMASA
 * Regla 30-30-30 con entrelazamiento, capas de fuego real (NASA FIRMS),
 * Ã¡reas quemadas reales (EFFIS), predicciÃ³n de propagaciÃ³n por viento
 * y asesor Manolito.
 */

// 0. CONSTANTES Y CONFIGURACIÃ“N
const DOM = {
    map: 'map',
...
// de FIRMS no estÃ¡ disponible (ej. al ejecutar sin 'wrangler dev').
...
/**
 * Utilidad para retrasar la ejecuciÃ³n de una funciÃ³n (debounce).
 * Evita que se llame a la API de Overpass en cada clic rÃ¡pido.
 */
...
baseSat.addTo(map); // capa por defecto (satÃ©lite, como en la imagen de referencia)

// Grupo de incendios (puntos FIRMS)
const grupoFuegos = L.layerGroup().addTo(map);

// Grupo del perÃ­metro estimado de incendios activos (se dibuja a partir
// de los propios puntos FIRMS agrupados, y crece/cambia segÃºn llegan
// detecciones nuevas del satÃ©lite)
const grupoPerimetroFuegos = L.layerGroup().addTo(map);

// Capa de Ã¡reas quemadas reales (EFFIS - Copernicus, gratis, sin API key)
...
const capasEffis = L.tileLayer.wms('https://maps.effis.emergency.copernicus.eu/effis', {
    ...
    attribution: 'EFFIS - Copernicus (actualizado por paso de satÃ©lite VIIRS, casi en tiempo real)'
});
...
const capasBase = {
    ...
    "TopogrÃ¡fica": baseTopo
};

const capasOverlay = {
    "Incendios activos - puntos (FIRMS)": grupoFuegos,
    "PerÃ­metro estimado - incendios activos": grupoPerimetroFuegos,
    "Ãreas quemadas reales (EFFIS)": capasEffis
};
...
let marcadorActivo = null;      // marcador de evaluaciÃ³n cuÃ¡ntica
let flechaViento = null;        // flecha de direcciÃ³n de viento
let conoPropagacion = null;     // polÃ­gono/cono de propagaciÃ³n estimada

// 2. SIMULADOR CUÃNTICO (3 Qubits)
...
// 3b. UTILIDADES DE VIENTO Y PROPAGACIÃ“N
...
// 4. EJECUCIÃ“N DEL MOTOR CUÃNTICO
function ejecutarMotorCuantico(lat, lon, temp, hum, wind, windDir, lugar, humedadSuelo, esDia) {
    ...
    DOM.logDetalles.innerHTML =
        `P(|011âŸ©) = ${detalles[3]}%<br>P(|101âŸ©) = ${detalles[5]}%<br>P(|110âŸ©) = ${detalles[6]}%<br>P(|111âŸ©) = ${detalles[7]}%`;
...
function actualizarInterfazYMapa(lat, lon, pct, temp, hum, wind, windDir, lugar, humedadSuelo, esDia) {
    ...
    if (perimetroEstimado && perimetroEstimado.dentro) {
        const avisoPerimetro = `âš  ESTE PUNTO ESTÃ DENTRO DE UN PERÃMETRO ESTIMADO DE INCENDIO ACTIVO (Ã¡rea aproximada del foco: ${perimetroEstimado.areaHa.toFixed(1)} ha). Prioridad mÃ¡xima: confirma con el 112 y no accedas a la zona sin coordinaciÃ³n con los servicios de extinciÃ³n.\n\n${textoRecomendacionFinal}`;
...
async function consultarOverpass(query, timeoutMs = 8000) {
    for (const url of OVERPASS_MIRRORS) {
        ...
        try {
            ...
            console.warn(`[Overpass] ${url} respondiÃ³ ${resp.status}, probando siguiente espejo`);
        } catch (e) {
            clearTimeout(timer);
            console.warn(`[Overpass] Fallo en ${url}:`, e.message, 'â€” probando siguiente espejo');
        }
    }
...
// 6b. CONTEXTO AMPLIADO: vegetaciÃ³n y agua cercana
async function obtenerContextoTerreno(lat, lon) {
    ...
    let tipoVegetacion = 'No determinado con precisiÃ³n (sin etiquetado detallado en OpenStreetMap para este punto)';
    if (especies.size > 0) {
        tipoVegetacion = Array.from(especies).join(', ');
    } else if ((data.elements || []).some(el => (el.tags || {}).natural === 'wood' || (el.tags || {}).landuse === 'forest')) {
        tipoVegetacion = 'Masa forestal genÃ©rica (sin especie detallada en OSM)';
    }

    return { vegetacion: tipoVegetacion, aguaCercana: hayAgua };
}

// 6c. FETCH CON REINTENTOS (clima) â€” para que nunca se quede colgado
async function fetchConReintentos(url, intentos = 3, timeoutMs = 6000) {
    for (let i = 0; i < intentos; i++) {
        ...
        try {
            ...
            console.warn(`[Clima] Intento ${i + 1} respondiÃ³ ${resp.status}`);
        } catch (e) {
            clearTimeout(timer);
            console.warn(`[Clima] Intento ${i + 1} fallÃ³:`, e.message);
        }
...
// 6. OBTENCIÃ“N DE DATOS METEOROLÃ“GICOS (clic en mapa)
    ...
    try {
        ...
        if (data.elevation <= 0) {
            ...
            marcadorActivo = L.circleMarker([lat, lon], {
                ...
                opacity: 0.9,
                fillOpacity: 0.6
            }).addTo(map).bindPopup(`<b>${t('popupZonaAgua')}</b><br><small>ElevaciÃ³n: ${data.elevation}m</small>`);
            window.ultimoContextoManolito = null;
            return;
        }
        ...
        DOM.uiTemp.textContent = `${temp} Â°C`;
        DOM.uiHum.textContent = `${hum} %`;
        DOM.uiWind.textContent = `${wind} km/h`;
        if (DOM.uiWindDir) DOM.uiWindDir.textContent = `${windDir}Â° (${gradosACardinal(windDir)})`;
...
// 7a. AGRUPACIÃ“N Y PERÃMETRO ESTIMADO A PARTIR DE PUNTOS ACTIVOS
// Esto NO es un dato inventado: se calcula en directo a partir de los propios
// puntos reales de FIRMS. Cada vez que llegan detecciones nuevas del satÃ©lite,
...
// Agrupa puntos cercanos entre sÃ­ (radio en metros) en focos independientes
...
// pase justo pegado a los puntos y se vea como un perÃ­metro real de incendio
...
function areaPoligonoHa(coordsLatLon) {
    ...
    return Math.abs(area) / 2 / 10000; // mÂ² -> ha
}
...
// Guarda la geometrÃ­a de todos los perÃ­metros activos dibujados, para poder
// consultar luego si un punto cualquiera cae dentro de alguno o a quÃ©
// distancia estÃ¡ del mÃ¡s cercano (lo usa el informe y la recomendaciÃ³n).
window.perimetrosActivosGeom = [];

// Dado un punto, dice si estÃ¡ DENTRO de un perÃ­metro activo estimado, o a
// quÃ© distancia (km) estÃ¡ del mÃ¡s cercano, y el Ã¡rea aproximada (ha) de ese foco.
    ...
    grupos.forEach(grupo => {
        if (grupo.length === 1) {
            ...
            L.circle([grupo[0].lat, grupo[0].lon], { ...estiloPerimetro, radius: radioM })
                ...
                .bindTooltip('PerÃ­metro estimado a partir de detecciones activas');
            ...
            L.polygon(hullExpandido, estiloPerimetro)
                .addTo(grupoPerimetroFuegos)
                .bindTooltip('PerÃ­metro estimado a partir de detecciones activas (crece con nuevas detecciones)');
            window.perimetrosActivosGeom.push({
...
async function cargarFuegosActivos() {
    ...
    try {
        ...
        const resp = await fetch(url);
        if (!resp.ok) throw new Error("FIRMS no respondiÃ³");
...
// 8. INTERACTIVIDAD DE LA INTERFAZ Y LOGICA LEGAL
        ...
        if (logHeader) {
            logHeader.addEventListener('click', () => {
                ...
                if (DOM.quantumLogSection.classList.contains('section-collapsed')) {
                    logHeader.innerHTML = "Registro cuantico >";
                } else {
                    logHeader.innerHTML = "Registro cuantico v";
                }
            });
